...
$manifest = '{"requiredResourceAccess":[{"resourceAppId":"00000003-0000-0000-c000-000000000000","resourceAccess":[{"id":"98830695-27a2-44f7-8c18-0c3ebc9698f6","type":"Role"}]}]}'
...
Write-Host "`n✅ Granting admin consent for GroupMember.Read.All..."
...
$alreadyGranted = $existing.value | Where-Object { $_.appRoleId -eq "98830695-27a2-44f7-8c18-0c3ebc9698f6" }

if ($alreadyGranted) {
    Write-Host "  ℹ️  Permission already granted — skipping."
} else {
    $body = "{`"principalId`":`"$spId`",`"resourceId`":`"$graphSpId`",`"appRoleId`":`"98830695-27a2-44f7-8c18-0c3ebc9698f6`"}"
